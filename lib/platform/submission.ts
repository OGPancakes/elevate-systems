...
function object(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new PlatformError(422, "INVALID_SUBMISSION", "A submission object is required.");
  return value as Record<string, unknown>;
}

function exactKeys(value: Record<string, unknown>, allowed: string[]) {
  if (Object.keys(value).some((key) => !allowed.includes(key)))
    throw new PlatformError(422, "UNKNOWN_FIELD", "The submission contains unsupported fields.");
}
...
function text(value: unknown, maximum: number): string {
  if (
    ...
    throw new PlatformError(422, "INVALID_SUBMISSION", "A required field is missing or invalid.");
  return value.trim();
}
...
export function validateSubmission(input: unknown, now = Date.now()): Submission {
  ...
  if (row.schema !== "consultation.v1" || !["virtual", "in-person"].includes(String(row.kind)))
    throw new PlatformError(
      422,
      "INVALID_SCHEMA",
      "This submission schema or kind is unsupported.",
    );
  const createdAt = text(row.createdAt, 40);
  if (
    !/^\d{4}-\d{2}-\d{2}T/.test(createdAt) ||
    !Number.isFinite(Date.parse(createdAt)) ||
    Date.parse(createdAt) > now + 300000
  )
    throw new PlatformError(422, "INVALID_TIMESTAMP", "A valid submission time is required.");
  const email = text(customer.email, 254).toLowerCase();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
    throw new PlatformError(422, "INVALID_EMAIL", "A valid email is required.");
  return {
    externalId: text(row.externalId, 128),
    externalCustomerId: text(row.externalCustomerId, 128),
    schema: "consultation.v1",
    ...
    customer: {
      ...
      email,
      phone: customer.phone == null ? null : text(customer.phone, 40),
    },
  };
}
...
export async function readSubmission(request: Request) {
  ...
  try {
    while (true) {
      ...
      if (size > 16384) {
        await reader.cancel();
        throw new PlatformError(413, "BODY_TOO_LARGE", "The submission is too large.");
      }
  ...
  try {
    ...
    throw new PlatformError(400, "INVALID_JSON", "The JSON body is invalid.");
  }
