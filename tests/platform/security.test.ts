...
import {
  ...
  verifyWebhook,
  type Credential,
} from "../../lib/platform/credentials.ts";
import {
  readSubmission,
  submissionDigest,
  validateSubmission,
} from "../../lib/platform/submission.ts";
...
const payload = {
  ...
  customer: { name: "Test Customer", email: "pilot@example.invalid", phone: null },
};
const denied = (error: unknown) =>
  error instanceof PlatformError && [401, 403, 404].includes(error.status);
...
test("business A user cannot select business B or access its customer", () => {
  ...
  assert.throws(
    () => assertOwnership(principal, { businessId: businessB, environment: "staging" }),
    denied,
  );
});
test("identity mismatch, absent, inactive and revoked memberships fail closed", () => {
  ...
  for (const field of ["active", "businessActive"])
    assert.throws(
      () => memberContext("user-a", businessA, "staging", { ...membership, [field]: false }),
      denied,
    );
});
test("Elevate staff also require an explicit membership for the selected business", () => {
  assert.throws(
    () => memberContext("user-a", businessB, "staging", { ...membership, role: "elevate_owner" }),
    denied,
  );
});
test("business members cannot access billing, team controls or create requests", () => {
  const member = memberContext("user-a", businessA, "staging", {
    ...membership,
    role: "business_member",
  });
  authorize(member, "read:customers");
  for (const scope of ["read:billing", "manage:team", "create:change-request"] as const)
    assert.throws(() => authorize(member, scope), denied);
});
test("credentials enforce business ownership, scope, environment, expiry and revocation", () => {
  ...
  for (const patch of [
    ...
    assert.throws(
      () => credentialContext(`Bearer ${token}`, { ...credential, ...patch }, "staging"),
      denied,
    );
  }
...
test("unknown environment and cross-environment reads fail closed", () => {
  ...
  assert.throws(
    () => assertOwnership(principal, { businessId: businessA, environment: "production" }),
    denied,
  );
});
test("intake rejects tenant injection, private photos and unsupported data", () => {
  for (const field of ["businessId", "photos", "reasonForWig", "metadata"])
    assert.throws(() => validateSubmission({ ...payload, [field]: "private" }));
  assert.throws(() =>
    validateSubmission({ ...payload, customer: { ...payload.customer, cardNumber: "test" } }),
  );
...
test("normalized retries have equal digests, changes have different digests", () => {
  ...
  assert.equal(
    submissionDigest(first),
    submissionDigest(validateSubmission(Object.fromEntries(Object.entries(payload).reverse()))),
  );
  assert.notEqual(
    submissionDigest(first),
    submissionDigest(validateSubmission({ ...payload, kind: "in-person" })),
  );
});
test("bounded input rejects chunked oversize bodies and invalid content type", async () => {
  await assert.rejects(
    ...
    (error: unknown) => error instanceof PlatformError && error.status === 413,
  );
  await assert.rejects(
    readSubmission(
      new Request("https://example.invalid", {
...
test("webhook verifies raw body, path, timestamp and secret", () => {
  ...
  const secret = "test-only-secret-value-with-32-bytes";
  const path = "/api/v1/webhooks/test";
  const signature =
    "sha256=" +
    createHmac("sha256", secret)
      .update([timestamp, "POST", path, createHash("sha256").update(body).digest("hex")].join("\n"))
      .digest("hex");
  ...
  for (const patch of [
    ...
    assert.equal(verifyWebhook({ ...input, ...patch }), false);
});
