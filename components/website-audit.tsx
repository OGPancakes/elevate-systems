    ...
    <section id="audit" className="section-pad mx-auto max-w-7xl px-5">
      <div className="mb-10 max-w-3xl">
        ...
        <p className="mt-5 max-w-2xl text-lg leading-8 text-white/55">
          Get a focused AI review and a visual redesign concept built around your business.
        </p>
      </div>
      ...
      <div className="glass grid overflow-hidden rounded-lg lg:grid-cols-[0.92fr_1.08fr]">
        <form className="space-y-4 border-b border-white/10 p-5 sm:p-7 lg:border-b-0 lg:border-r" onSubmit={submitAudit}>
          ...
          <details className="border-t border-white/10 pt-4">
            <summary className="cursor-pointer text-sm text-white/45 transition hover:text-white/70">
              Add a logo for a more branded concept (optional)
            </summary>
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <label className="space-y-2">
                <span className="text-sm font-medium text-white/60">Logo URL</span>
                <div className="flex items-center gap-2 rounded-md border border-white/10 bg-black/30 px-3 py-3 ring-sky-300/40 focus-within:ring-2">
              ...
              <label className="space-y-2">
                <span className="text-sm font-medium text-white/60">Upload logo</span>
          ...
          <div className="relative">
            <div className="mb-6 border-b border-white/10 pb-6">
              ...
              <p className="mt-3 leading-7 text-white/60">
                {websiteUrl.trim()
                  ? `Ready to review ${websiteUrl.trim()} for clarity, trust, and conversion opportunities.`
                  : "Enter your details to unlock the audit and redesign preview."}
              </p>
            </div>
            ...
            {result ? (
              <div className="space-y-4">
                ...
                {[
                  ["What is working", result.strengths],
                  ["Where the site loses momentum", [...result.weaknesses, ...result.improvements]],
                  ["The clearest next move", result.howElevateHelps]
                ].map(([title, items]) => (
                  <details className="group border-b border-white/10 py-5" key={String(title)} open={title === "What is working"}>
              ...
              <div className="space-y-5 pt-3">
                {[
                  "A concise review of what helps or hurts conversion",
                  "A premium homepage redesign concept",
                  "A practical recommendation for what to improve first"
                ].map((label) => (
                  <div className="flex items-start gap-3" key={label}>
                    <CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-sky-300" />
                    <p className="leading-7 text-white/55">{label}</p>
                  </div>
                ))}
              </div>
            )}
