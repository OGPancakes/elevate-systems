...
export async function POST(request: Request) {
  return endpoint(async (requestId) => {
    ...
    try {
      ...
      throw new PlatformError(400, "INVALID_LOGIN", "Email and password are required.");
    }
    if (
      !input ||
    ...
    if (!result.user.email_confirmed_at || !result.access_token)
      throw new PlatformError(401, "UNAUTHENTICATED", "Please verify your email.");
    (await cookies()).set(sessionCookie, result.access_token, {
      httpOnly: true,
      secure: configuration().environment !== "local",
      sameSite: "strict",
      path: "/",
...
export async function DELETE(request: Request) {
  return endpoint(async (requestId) => {
    ...
    if (token) {
      try {
        await provider("auth/v1/logout?scope=local", { token, method: "POST" });
      } catch (error) {
        if (!(error instanceof PlatformError) || error.status !== 401) throw error;
      }
    }
    jar.delete(sessionCookie);
    return json({ signedOut: true }, requestId);
  });
}
