...
export async function POST(request: Request) {
  return endpoint(async (requestId) => {
    ...
    if (principal.actorType !== "credential")
      throw new PlatformError(
        403,
        "ACCESS_DENIED",
        "Submission intake requires a server credential.",
      );
    ...
    const accepted = await provider<{
      ...
      createdAt: string;
      duplicate: boolean;
    }>("rest/v1/rpc/platform_accept_submission", {
      admin: true,
      method: "POST",
      body: {
        p_credential: principal.actorId,
        p_business: principal.businessId,
        p_environment: principal.environment,
        p_payload: submission,
        p_hash: submissionDigest(submission),
      },
    });
...
export async function GET(request: Request) {
  return listResource(request, "submissions");
}
