  ...
  return (
    <>
      ...
      <form className="mb-6 grid gap-3 rounded-lg border border-white/10 bg-white/[0.03] p-3 sm:grid-cols-[1fr_180px_auto]" action="/admin/inquiries">
        <input className="rounded-md border border-white/10 bg-black/30 px-3 py-2.5 text-sm text-white outline-none focus:border-sky-300/50" defaultValue={params?.q} name="q" placeholder="Search inquiries" />
          ...
          <div className="divide-y divide-white/10">
            {filtered.map((item) => (
              <div className="grid gap-4 px-5 py-4 sm:grid-cols-[1fr_160px_auto]" key={item.id}>
                ...
                <span className="text-xs text-white/40">{new Date(item.submitted_at).toLocaleString()}</span>
              </div>
            ))}
          </div>
